...
class VisualBertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if token_type_ids is None:
            ...
            #token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=self.inputs_embeds.device)
...
class VisualBertModel(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states=None, # Newj
        return_dict=None,
    ):
        r"""
        ...
        #print("modeling 799 - attention_mask", attention_mask, attention_mask.shape)
        #print("modeling 800 - visual_attention_mask", visual_attention_mask, visual_attention_mask.shape)
...
class VisualBertForPreTraining(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states=True, # Newj - added to get HS for all layers
        return_dict=None,
        labels=None,
...
class VisualBertForQuestionAnswering(VisualBertPreTrainedModel):
    ...
    def forward(
        ...
        # Newj - add preds
        vb_preds = torch.argmax(reshaped_logits, dim=-1)
        vb_preds = torch.flatten(vb_preds)
...
def v_b_w(x):
  if isinstance(x, nn.Conv2d):
    ...
    torch.nn.init.normal_(x.bias, 0.0, 0.02)



@add_start_docstrings(
    """
    VisualBert Model with a visual boost layer and a classification/regression head on top (a dropout and a linear layer on top of the pooled
    output) for VLN.
    """,
    VISUAL_BERT_START_DOCSTRING,
)
class VBModelforFLrl(VisualBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.visual_bert = VisualBertModel(config)
    ...
    def forward(
        ...
        vbm_hidden_states = outputs[2]

        return vbm_hidden_states


@add_start_docstrings(
    """
    VisualBert Model for VLN.
    """,
    VISUAL_BERT_START_DOCSTRING,
)
class VBforVLN(VisualBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.visual_bert = VisualBertModel(config)
    ...
    def forward(
        ...
        token_type_ids = torch.ones(inputs_embeds.shape[:-1], dtype=torch.long).to(torch.device('cuda'))
        attention_mask = torch.ones(inputs_embeds.shape[:-1], dtype=torch.long).to(torch.device('cuda'))
        
        visual_token_type_ids = torch.ones(visual_embeds.shape[:-1], dtype=torch.long).to(torch.device('cuda'))
        visual_attention_mask = torch.ones(visual_embeds.shape[:-1], dtype=torch.long).to(torch.device('cuda'))
